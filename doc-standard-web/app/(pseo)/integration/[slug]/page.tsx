...
export async function generateStaticParams() {
  ...
  const rawLimit = process.env.PSEO_INTEGRATION_STATIC_LIMIT ?? "0"
  const limit = Number.parseInt(rawLimit, 10)

  // Default is runtime-only generation to avoid build memory spikes in constrained containers.
...
export default async function IntegrationFactoryPage({ params }: PageProps) {
  ...
  return (
    <div className="bg-white text-slate-900 leading-relaxed">
      ...
      {/* NEW: Expert Technical Analysis Section (Bulk Content) */}
          ...
          <div className="grid lg:grid-cols-3 gap-12">
            ...
            <div className="lg:col-span-2">
              <div className="space-y-8">
                {model.technicalGuide.steps.map((step, i: number) => (
