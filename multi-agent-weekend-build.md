...
## 3. Heartbeat API Route

### File: `doc-standard-web/app/api/ops/heartbeat/route.ts`
...
### Cron Setup (on your Mac):

Add to crontab:
```bash
*/5 * * * * curl -s -H "Authorization: Bearer YOUR_CRON_SECRET" https://docstandard.co/api/ops/heartbeat
```

Or use a simple Node.js scheduler on your Mac:
```javascript
// workers/scheduler.js
setInterval(() => {
  fetch('https://docstandard.co/api/ops/heartbeat', {
